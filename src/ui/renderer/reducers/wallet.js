...
export const getWallets = () => async dispatch => {
  ...
  try {
    ...
    //const zapWallets1 = allWallets(map(w => w.type !== 'fullNodeWallet'))
    const fullNodeWallets = allWallets.filter(w => {
      if (w.type === 'fullNodeWallet') {
        return w
      }
    })
    const zapWallets = allWallets.filter(w => {
      if (w.type !== 'fullNodeWallet') {
        return w
      }
    })
    wallets = await Promise.all(zapWallets.map(window.Zap.generateLndConfigFromWallet))
    all = fullNodeWallets.concat(wallets)
  } catch (e) {
    wallets = []
  }
  dispatch(setWallets(all))
  return all
}
