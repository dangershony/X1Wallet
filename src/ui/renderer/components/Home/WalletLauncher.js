...
import X1WalletSettingsFormFullNode from '../../../obsidian/X1WalletSettingsFormFullNode'
import {
  callLoadWallet,
  loadWalletIntoStore,
  getFullNodeKey,
} from '../../../obsidian/reducers/fullNodeConnectionActions'
...
class WalletLauncher extends React.Component {
  ...
  launchWallet = async () => {
    const { startLnd, wallet, history } = this.props
    try {
      if (wallet.type === 'fullNodeWallet') {
        let fullNodeWallet = JSON.parse(wallet.alias)
        window.x1Store.dispatch(getFullNodeKey())
        window.x1Store.dispatch(loadWalletIntoStore(fullNodeWallet))
        window.x1Store.dispatch(callLoadWallet(fullNodeWallet))

        // info.chain and and info.network will be set from the first item in the array
        const info = {
          chains: [{ chain: 'obsidianx', network: 'mainnet' }],
          testnet: false,
          synced_to_chain: false,
        }

        window.x1Store.dispatch(setInfo(info))
        history.push('/xapp')
      } else {
    ...
    try {
      ...
      if (wallet.type === 'fullNodeWallet') {
        const fullNodeWallet = {
          fullNodeHost: values.fullNodeHost,
          fullNodeUser: values.fullNodeUser,
          fullNodePwd: values.fullNodePwd,
          fullNodeWalletFilePath: values.fullNodeWalletFilePath,
          fullNodeWalletNames: values.fullNodeWalletNames,
          fullNodeSelectedWalletName: values.name,
          fullNodeExpectedAuthKey: values.fullNodeExpectedAuthKey,
        }
        result.alias = JSON.stringify(fullNodeWallet)
      } else if (wallet.type === 'local') {
  ...
  hasChanges = () => {
    ...
    // check for changes in the Full Node settings
    if (wallet.type === 'fullNodeWallet') {
      if (!formState.dirty) {
        return false
      }
      let fnwHasChanges = false
      let fullNodeWallet = JSON.parse(wallet.alias)
      if (formState.values.fullNodeUser !== fullNodeWallet.fullNodeUser) {
        fnwHasChanges = true
      }
      if (formState.values.fullNodePwd !== fullNodeWallet.fullNodePwd) {
        fnwHasChanges = true
      }
      if (formState.values.name !== fullNodeWallet.fullNodeSelectedWalletName) {
        fnwHasChanges = true
      }
      return fnwHasChanges
    }
  ...
  render() {
    ...
    if (wallet.type === 'fullNodeWallet') {
      try {
        var fullNodeWallet = JSON.parse(wallet.alias)
        walletConverted = {
          alias: wallet.alias,
          autopilot: false,
          chain: wallet.chain,
          ...
          fullNodeHost: fullNodeWallet.fullNodeHost,
          fullNodeUser: fullNodeWallet.fullNodeUser,
          fullNodePwd: fullNodeWallet.fullNodePwd,
          fullNodeWalletFilePath: fullNodeWallet.fullNodeWalletFilePath,
          fullNodeWalletNames: fullNodeWallet.fullNodeWalletNames,
          fullNodeSelectedWalletName: fullNodeWallet.fullNodeSelectedWalletName,
          fullNodeExpectedAuthKey: fullNodeWallet.fullNodeExpectedAuthKey,
        }
      } catch (e) {
        walletConverted = {
          alias: 'Invalid JSON: ' + e,
          autopilot: false,
          chain: wallet.chain,
          id: wallet.id,
          name: wallet.name + ' (Invalid JSON)',
          network: wallet.network,
          type: wallet.type,
          wallet: wallet.wallet,
        }
      ...
      walletConverted = walletToFormFormat(wallet)
    }
