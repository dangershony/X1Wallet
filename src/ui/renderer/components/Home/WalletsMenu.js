...
class WalletsMenu extends React.Component {
  ...
  render() {
    ...
    const fullNodeWallets = wallets.filter(wallet => wallet.type === 'fullNodeWallet')
    const localWallets = wallets.filter(wallet => wallet.type === 'local')
    const otherWallets = wallets.filter(
      wallet => wallet.type !== 'local' && wallet.type !== 'fullNodeWallet'
    )
    ...
    return (
      <Box {...rest}>
        <WalletGroup
          ...
          title="Full Node Wallets"
          wallets={fullNodeWallets}
        />
        <WalletGroup
          activeWallet={activeWallet}
          mb={5}
          setActiveWallet={setActiveWallet}
          title="Bitcoin Lightning Wallets"
