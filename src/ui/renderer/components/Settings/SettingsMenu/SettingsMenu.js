...
import xmessages from '../../../../obsidian/components/XProfile/xmessages'
...
const SettingsMenu = ({
  ...
  const title = window.x1Tools.isZap() ? messages.profile_title : xmessages.advanced_page_title

  const openProfile = () => {
    if (window.x1Tools.isZap()) {
      return openModal('PROFILE')
    } else {
      return openModal('XPROFILE_MODAL')
    }
  }
