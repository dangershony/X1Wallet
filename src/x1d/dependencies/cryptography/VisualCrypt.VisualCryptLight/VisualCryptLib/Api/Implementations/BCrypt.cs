...
namespace VisualCrypt.VisualCryptLight.VisualCryptLib.Api.Implementations
{
    public sealed class BCrypt
    {
        ...
        BCrypt(LongRunningOperationContext context)
        {
            ...
            this._encryptionProgress = context.EncryptionProgress;
        ...
        void Encipher(uint[] blockArray, int offset)
        {
            ...
            unchecked
            {
                for (round = 0; round <= BlowfishNumRounds - 2;)
                {
                    ...
                    n = this._s[(block >> 24) & 0xff];
                    n += this._s[0x100 | ((block >> 16) & 0xff)];
                    n ^= this._s[0x200 | ((block >> 8) & 0xff)];
                    n += this._s[0x300 | (block & 0xff)];
                    r ^= n ^ this._p[++round];
                    ...
                    block ^= n ^ this._p[++round];
                }
                blockArray[offset] = r ^ this._p[BlowfishNumRounds + 1];
        ...
        void InitializeKey()
        {
            this._p = new uint[POrig.Length];
            this._s = new uint[SOrig.Length];
            Array.Copy(POrig, this._p, POrig.Length);
            Array.Copy(SOrig, this._s, SOrig.Length);
        }
        ...
        void Key(byte[] keyBytes)
        {
            ...
            int koffp = 0;
            uint[] lr = { 0, 0 };
            int plen = this._p.Length, slen = this._s.Length;

            for (i = 0; i < plen; i++)
                this._p[i] = this._p[i] ^ StreamToWord(keyBytes, ref koffp);
        ...
        void EksKey(byte[] saltBytes, byte[] inputBytes)
        {
            ...
            for (i = 0; i < plen; i++)
                this._p[i] = this._p[i] ^ StreamToWord(inputBytes, ref passwordOffset);
        ...
        byte[] CryptRaw(byte[] inputBytes, byte[] saltBytes, int logRounds)
        {
            ...
            for (int i = 0; i < rounds; i++)
            {
                ...
                this._encryptionProgress.Percent = (int)progressValue;
                this._encryptionProgress.Report(this._encryptionProgress);
