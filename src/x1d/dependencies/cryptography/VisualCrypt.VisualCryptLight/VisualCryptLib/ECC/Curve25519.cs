...
namespace VisualCrypt.VisualCryptLight.VisualCryptLib.ECC
{
    sealed class Curve25519
    {
        ...
        static void Core(byte[] publicKey, byte[] signingKey, byte[] privateKey, byte[] peerPublicKey)
        {
            ...
            if (publicKey.Length != 32) throw new ArgumentException(String.Format("currentPublicKey must be 32 bytes long (but was {0} bytes long)", publicKey.Length), "publicKey");
